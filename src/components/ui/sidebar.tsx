...
function readCookieOpen(defaultOpen: boolean) {
...
>(function SidebarProvider(
  ...
  // SSR-safe initial read from cookie, only if uncontrolled
  const uncontrolledInitial = React.useMemo(
    () => readCookieOpen(defaultOpen),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    []
  );

  const [_open, _setOpen] = React.useState(uncontrolledInitial);
...
>(function Sidebar(
  ...
  return (
    <div
      ...
      data-side={side}
      aria-hidden={state === "collapsed" && collapsible === "offcanvas" ? true : false}
    >
      {/* Gap handler */}
      <div
        className={cn(
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          "absolute inset-y-0 z-20 flex w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] group-data-[side=left]:-right-4 group-data-[side=right]:left-0 hover:after:bg-sidebar-border",
  ...
  return (
    ...
    >
      ...
      <Skeleton
        ...
        style={{ ["--skeleton-width" as any]: width }}
      />
    </div>
  );
});
